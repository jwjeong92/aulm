...
def get_act_scales(model, tokenizer, smoothquant_dataset, num_samples=512, seq_len=512, args=None):
    ...
    if smoothquant_dataset=='pile':
        dataset_path=f'{args.cache_dir}/dataset/pile.val.jsonl.zst'
        if not os.path.exists(dataset_path):
            os.makedirs(f'{args.cache_dir}/dataset',exist_ok=True)
            import requests
            url = 'https://huggingface.co/datasets/mit-han-lab/pile-val-backup/resolve/main/val.jsonl.zst' # Pile
